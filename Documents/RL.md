...
2) **Policy** $\pi$ is a function that tells an agent which is the best action to choose in each state. A policy can be **deterministic** or **stochastic**.
    - **Deterministic Action**
        - For each action in a given state, there is exactly ONE possible outcome with a probability of 100%.
    - **Non-deterministic (Stochastic) Action**
        - For an action in a given state, there are multiple possible outcomes, each with its own probability.
    ---
...
2) **Expected discounted reward**
    ...
    - What does i mean?
        ...
        The **i** is the **time step** (or step number) in the trajectory.

        When you follow a policy starting from state s, you create a sequence of states, actions, and rewards over time:

        - **i = 0**: You're at state $s_0$ (the starting state s), take action $a_0$, get reward $r(s_0, a_0, s_1)$, move to state $s_1$
        - **i = 1**: You're at state $s_1$, take action $a_1$, get reward $r(s_1, a_1, s_2)$, move to state $s_2$
        - **i = 2**: You're at state $s_2$, take action $a_2$, get reward $r(s_2, a_2, s_3)$, move to state $s_3$
        - **i = 3**: ...and so on

    - The sum over i:

        $$\sum_{i} \gamma^i r(s_i, a_i, s_{i+1})$$

        This means you're adding up:

        $$\gamma^0 \cdot r(s_0, a_0, s_1) + \gamma^1 \cdot r(s_1, a_1, s_2) + \gamma^2 \cdot r(s_2, a_2, s_3) + ...$$

        So **i indexes the time steps** as you move through the environment following your policy. Each step, you get a reward, and that reward is discounted by $\gamma^i$ (rewards further in the future count less).

    - What does $E_{\pi}$ means:
    
        $E_{\pi}$ means the expectation over all possible trajectories generated by following policy π, accounting for both the randomness in action selection (from π) and the randomness in state transitions (from the environment).
